...
description: Main Codex 并行编排工作流：先把用户需求理解透彻并拆解为可执行 CSV TODO；默认优先在当前会话内用 `spawn_agent`/`wait_agent` 调度并行子代理，只有在用户明确要求 external worktree / 多终端多 Codex 时才切换为外部协作模式。适用于多文件独立改动、信息收集+分析、评审/调试/验证并行推进。触发词：并行施工、并行调度、collab、多 agent、parallel、CSV TODO。
---

# Codex 并行施工（Parallel Collab）

> 核心定位：Main Codex 负责深度理解需求与拆解；默认在当前会话内调度并行子代理执行与汇总；只有在用户明确要求 external worktree / 多终端多 Codex 时，才输出外部 orchestrate 方案。
...
- Main Codex 直接使用 `spawn_agent` 派发子任务
- 使用 `wait_agent` 等待子代理结果
- 保存返回的 `agent_id` / target
- 维护 `pending` 集合循环等待，直到全部完成
- 子代理完成且不再需要后，用 `close_agent` 回收

### 2) External worktree / 多 Codex 模式（仅显式切换）

- 仅在用户明确要求 external worktree / 多终端多 Codex / 独立工作分支时使用
- Main Codex 输出任务包、worktree 命令与子 Codex 提示词
- 该模式属于人工 orchestrate：主 agent 不持有运行时 agent id
- 不得把该模式描述成“当前会话已自动并行并自动等待”
...
- **当前会话内并行调度（默认）**：`spawn_agent`、`wait_agent`、`send_input`、`close_agent`
- **等待规则**：`wait_agent` 不保证一次返回全部结果，也不保证返回顺序；必须循环直到 `pending` 为空
- **外部协作模式（仅显式切换）**：输出 worktree 方案、提示词与任务包；不宣称自动等待
...
4. **按依赖分批并行调度**
   - 默认：仅调度当前会话内 `depends_on` 已满足的行，并使用 `spawn_agent` 并行下发
   - 仅当显式切换 external 模式时，输出 worktree + prompt 包给外部 Codex
5. **汇总与决策**
